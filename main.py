...
server = StreamingServer(localIPAddress, 9999)
reciever = AudioReceiver(localIPAddress, 8888)
...
def StartCameraStream():
    camera_client = CameraClient(textTargetIP.get(1.0,'end-1c'), 6000)
    t3 = threading.Thread(target = camera_client.start_stream)
    t3.start()
    
def StartScreenSharing():
    screen_sharing = ScreenShareClient(textTargetIP.get(1.0,'end-1c'), 6000)
    t4 = threading.Thread(target = screen_sharing.start_stream)
    t4.start()
    
def StartAudioStream():
    audio_sender = AudioSender(textTargetIP.get(1.0,'end-1c'), 7000)
